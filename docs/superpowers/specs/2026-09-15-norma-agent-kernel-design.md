...
| `read_file(path, offset=0, limit=8000)` | read | 读文本的**一个区间**（v1.1.0 起）。`offset` 按**字符**、0 起算；`limit` 可往小调、不可往大调（见下） |
...
**`read_file` 曾经有第三个参数 `max_bytes`，v1.0.1 删掉了。** 它是个假旋钮：真正的闸门是下面那层 8000 字符的结果截断，而它**在 `max_bytes` 之后**才生效，于是对中文（3 字节/字）`max_bytes` 超过约 24000 就完全无效。实测模型为此试了 6万/20万/40万字节、返回一模一样，白烧数步。

### `read_file` 的区间读取（v1.1.0）

**问题不是"上限 8000 太小"，是"没有游标"。** 实测：16848 字符的文件，模型调了 **3 次** `read_file`（3 × 8000 = 24000 > 16848，够覆盖全文），却只读到开头一半——因为每次都从第 0 个字符开始。**线性读取没有游标时，重试同一个工具永远在原地打转**，把上限翻多少倍都一样。

三条设计决定：

1. **`offset` 按字符，不按字节。** 模型脑子里数的是字符（它的原话是"没有 offset 参数"）；字节语义会让它算不准，还会把刚清掉的那类"切在多字节字符中间"的 bug 请回来。
2. **给游标，不给宽度。** `limit` 可以往小调、**不能往大调**——它被夹在 `MAX_RESULT_CHARS` 上。让模型把这个出口顶开，等于把上下文窗口的控制权交给它，**那正是 `max_bytes` 的同一个错误**。想多读就翻页。
3. **返回值开头带进度标记**：`…[第 1-7969 字符，共 16848 字符，后面还有]`。模型据此自己判断有没有下一段、下一个 `offset` 填多少。**标记必须在开头**——结果还要过 §9 ⑤ 那层只保留前 8000 字符的截断，写在末尾等于没写（v1.0.1 的终局审查正好抓到过这个错的另一种形态）。

   **标记是模型唯一的停止信号，所以它不能沉默。** 两种"到头了"必须分开说：

   | 情况 | 标记尾注 | 为什么 |
   |---|---|---|
   | 文件本身到头 | 不加 | 模型该收尾了 |
   | 只是**可读部分**到头（超 64 KiB） | `，后面读不到了` | 不加的话，一个照 prompt 办的模型会拿终点当 `offset` 再试一次——而那一次注定是"偏移超出文件长度"（实测 120000 字节的文件白烧一步） |

   同理，`offset` 越过可读末尾时，那句错误里也要带上"文件共 N 字节，只读了开头 M 字节"：
   那是模型对该文件的**最后一次**读，只看到干巴巴一个"共 24000 字符"会以为文件就这么长。

两条实现上的硬约束：

- **标记占额度，且额度从总上限扣、不从 `limit` 扣。** 从 `limit` 扣的话 `limit=5` 会被标记吃光、返回空内容；不扣总上限的话正文会被 `truncate()` 啃掉，而标记里"到第 N 字符"就成了假话——模型据此算下一个 offset 会**跳字或重复**。
- **整个文件一次给完时不加标记。** 小文件没有"是哪一段"的疑问，给它套一层壳只是让每次读配置都多跳一句废话。

内部 `_READ_CAP_BYTES = 64000` 保留：它是缓冲区，不是承诺，负责把文件按字节切开（那是"末尾半个字符是不是我们自己切的"这个判断的前提，见 §8 的 `_decode_or_none`）。当它咬住时，标记会一并报出文件真实**字节**数，免得模型把读到的那段长度当成整个文件。**`offset` 落在 64000 字节之后读不到**——这是已知边界，升级路径见 §14。
...
配套的两句：截断标记里的"原文 N 字符"报的是**工具返回的那段文本**的长度；工具自己的分段标记（如 `read_file` 的页标记）**也占这 8000 的额度**，因为它必须留在开头才不会被截断层吃掉。

**为什么不把这 8000 调大**（1.1.0 时算过，结论是不动）：8000 中文字符 ≈ 4800 token（DeepSeek 官方口径 1 中文字符 ≈ 0.6 token），占 `deepseek-chat` 128K 窗口的 **3.75%**——它从来没接近过自己的天花板，所以"8000"一直是看着合理而非算出来的。真要改，先让它读一章、记 `usage.prompt_tokens`（0.6 只是估算）再定；**而且那是重构**：`MAX_RESULT_CHARS` 同时管着 `run_powershell` 的洪水输出，读文件想放宽就得**按工具区分**，不是调这个全局常数。
...
- **只插一次，判据是 `messages[0]` 本来就是 system 消息**（不是"`messages` 里有没有"）。看位置而非存在，是因为 `messages[0]` 就是这条不变量本身：一个把历史恢复进 `messages`、system 却在中间的客户端，按"存在即跳过"会让默认 prompt 被整个跳掉——那正是 v2 恢复会话历史时的形态，中文提问先用英文答的缺陷会悄悄回来。客户端自己带的 system 消息（包括一条空的）照样优先，`messages` 是唯一事实来源；也不必维护第二份状态。
- **prompt 与工具描述必须互相一致**：模型在**两个不同时刻**读到它们（描述每轮随 schema 下发，prompt 在对话最前面）。实测教训成对出现：v1.0.1 补了"读取上限是多少"（否则它去试参数），v1.1.0 补了"怎么翻页"（否则它有了 `offset` 也还是反复读开头）。加能力时必须回头看 prompt 里那句是不是该跟着改。
...
| 取舍 | 代价 | 升级路径 |
|---|---|---|
| `read_file` 内部 64 KiB 上限（不可调） | 超过 64 KiB 的纯文本尾部读不到（此前也读不到，因为出口只有 8000 字符）。**现在至少会说清楚**（`文件共 N 字节，只读了开头 M 字节`），不会让模型把读到的那段长度当成文件大小 | 与 `offset` 同批解决；或改流式解码 |
| 日志固定 `~/.norma/audit.log`，无轮转 | 长期使用会持续增长 | 接入 `RotatingFileHandler`，或客户端提供查看入口 |
| `NORMA_LOG_PATH` 未进 README | 用户不知道有逃生舱（它是测试与调试用的隔离开关，不是用户配置项） | 真有第二个实例/沙箱需求时再文档化 |
| 测试里 `scripted()` 默认关掉 system 消息 | **"只在有 system 消息时才坏"的回归，可见面只有 7 条用例**（6 条单元 + 1 条真子进程 CLI）；把它们逐个改成 `[1:]` 视角则会给一堆无关断言加噪音 | 若真出现这类回归，把 `scripted()` 默认翻回开启并接受索引漂移 |
...
v1.1.0（`read_file` 区间读取）新增的边界：

| 取舍 | 代价 | 升级路径 |
|---|---|---|
| `offset` 按字符数，需要顺序解码定位 | `offset` 很深的读取要解码整块（64 KiB 内是毫秒级，但线性） | 流式解码 + 字节游标；或按行号寻址 |
| 读取仍受内部 64 KiB 上限 | **字符位置在 64000 字节之后的区间读不到**（对按 8000 字符切好的章文件无影响：一章 8-30 KB）。现在每一页都标着"只读了开头 M 字节"，翻到可读末尾还会明说"后面读不到了"，不会默默给一段短的、也不会让模型去追一个读不到的尾部 | 流式解码；或按 `offset` 直接 seek（要处理字符边界） |
| 单次仍最多 8000 字符（含标记） | 想"一次读完长文件"做不到，必须翻页 | `MAX_RESULT_CHARS` 按工具区分（读文件放宽、`run_powershell` 守 8000）——**先量 `usage.prompt_tokens` 再定数值**，见 §8 |
| `limit` 能被模型调小、不能调大 | 无实际代价（只会让它读少一点） | — |
| 区间按字符、不按行 | 要"读第 12 章"得自己换算字符位置（但它本来就是靠字符数推的） | 行号参数——等真实需求，别猜 |
| `read_file` 的产物**不再**会带 `truncate` 的截断标记（v1.1.0 起，v1.1.1 起有断言保证） | 需要靠"额度算准"这条不变量撑着：一旦预算估短，收尾截断就会啃掉正文，而页标记声明的区间就成了假话。v1.1.1 把两趟估算改成一趟，并留了断言 | 断言的另一种形态是直接抛异常给调用方（更响但更难看）；现在是 assert，`python -O` 下会静默失效——**代价已知、范围已用 166617 次扫描验证为 0** |
...
- **主要用途不是办公文档，而是辅助游戏开发**（根据小说改编）。含义：要读大文件、要跑多步骤长任务、要写代码并执行构建。
  - **立刻会撞到的墙**：`read_file` **一次只给出 8000 字符，而且只能从文件开头读**（只能读一次开头，没有游标）——所以一部小说读不完，单章稍长也要分好几次、而每次拿到的还是同一段。这是 v2 之前就要面对的具体缺口。（内部那个 64 KiB 读取上限不是这里的瓶颈：它只是缓冲区大小，真正跟模型见面的是 8000 字符那道出口。见 §8 与 §14。）
